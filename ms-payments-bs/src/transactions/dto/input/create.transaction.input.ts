...
export class CreateTransactionInput {
  ...
  @IsUUID(UUID_VERSION, {
    message: 'accountExternalIdDebit must be a valid UUID',
  })
  accountExternalIdDebit: string;

  @Field({ description: 'Account External ID for Credit' })
  @IsUUID(UUID_VERSION, {
    message: 'accountExternalIdCredit must be a valid UUID',
  })
  accountExternalIdCredit: string;

  @Field({ description: 'Transfer Type ID' })
  @IsValidTransferTypeId({
    message: 'transferTypeId must be a valid transaction type ID (1, 2, or 3)',
  })
